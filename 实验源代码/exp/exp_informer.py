import pandas as pd
# from data.data_loader_with_timeboader import Dataset_ETT_hour, Dataset_ETT_minute, Dataset_Custom, Dataset_Pred
...
class Exp_Informer(Exp_Basic):
    ...
    def predict(self, args, setting, load=False):
        ...
        def append_and_save_npy(file_path, new_data):
            new_data_flatten = new_data.flatten()  # Flatten the array to 1D
            if os.path.exists(file_path):
                existing_data = np.load(file_path, allow_pickle=True)
                combined_data = np.concatenate([existing_data, new_data_flatten])
            else:
                combined_data = new_data_flatten
        ...
        if args.is_rolling_predict:
            ...
            print("进行了滚动预测！")
            print("pre_data路径为",args.root_path + args.data_path)
        else:
            pre_data = pd.read_csv(args.root_path + args.data_path)
            print("没有进行滚动预测！")
            print("pre_data路径为", args.root_path + args.data_path)
        ...
        for i in range(int(len(pre_data)/pre_length)):
            ...
            # 存储 preds 到 CSV 文件
            with open(folder_path + 'pred_Predict.csv', 'a') as file:
                for i in range(preds.shape[0]):
                    np.savetxt(file, preds[i], delimiter=',')

            # 存储 trues 到 CSV 文件
            with open(folder_path + 'true_Predict.csv', 'a') as file:
                for i in range(trues.shape[0]):
                    np.savetxt(file, trues[i], delimiter=',')

            # 存储指标数据到 CSV 文件
            with open(folder_path + 'results_Indicator_Predict.csv', 'a') as file:
                np.savetxt(file, np.array([mae, mse, rmse, mape, mspe]).reshape(1, -1), delimiter=',')
